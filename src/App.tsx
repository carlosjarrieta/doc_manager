    ...
    <div className="flex h-screen overflow-hidden font-display">
      ...
      <aside className="w-64 border-r border-slate-200 bg-white flex flex-col z-10 shrink-0">
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4">
          ...
          <Link to="/" className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-[var(--color-primary-10)] text-[var(--color-primary)] font-bold transition-colors">
        ...
        <header className="h-16 border-b border-slate-200 bg-white px-8 flex items-center justify-between shrink-0 relative z-10 shadow-sm">
          <div className="flex-1 max-w-xl">
            <div className="relative group">
              ...
              <input className="w-full pl-10 pr-4 py-2 bg-slate-50 border-none outline-none rounded-lg focus:ring-2 focus:ring-[var(--color-primary)]/20 text-sm font-semibold text-slate-700 placeholder:text-slate-400 transition-all" placeholder="Search documents, tags, or shared files..." type="text" />
            </div>
          </div>
          <div className="flex items-center gap-4 ml-8">
            <Link to="/" className="flex items-center gap-2 bg-[var(--color-primary)] text-white px-4 py-2.5 rounded-lg font-bold text-sm hover:opacity-90 transition-all shadow-sm">
              <ScanLine className="w-4 h-4" /> Scan New Document
            </Link>
        ...
        <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-slate-200 bg-white px-10 py-4 shadow-sm relative z-10">
          <div className="flex items-center gap-8">
            ...
            <div className="hidden md:flex relative group min-w-40 max-w-64 h-10">
              <div className="flex w-full flex-1 items-stretch rounded-lg h-full bg-slate-100">
                ...
                <input className="flex w-full min-w-0 flex-1 border-none bg-transparent focus:ring-0 h-full placeholder:text-slate-500 px-2 pl-0 text-sm font-semibold outline-none text-slate-700" placeholder="Search codes..." />
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-1 justify-end gap-8 items-center">
            <nav className="flex items-center gap-8">
              ...
              <Link className="text-[var(--color-primary)] border-b-2 border-[var(--color-primary)] py-1 text-sm font-bold" to="#">Verify</Link>
            </nav>
...
const AppContent = () => {
  ...
  return isPublicPage ? (
    ...
    <AdminLayout>
      <Routes>
        <Route path="/" element={<DocumentRegister />} />
      </Routes>
    </AdminLayout>
  );
};
